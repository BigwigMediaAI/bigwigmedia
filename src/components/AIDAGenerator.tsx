...
export function AidaFramework() {
  ...
  const [generatedAIDA, setgeneratedAIDA] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateAIDA?clerkId=${userId}`, {
        companyName,
        productDescription,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedAIDA(response.data);
      } else {
        toast.error('Error generating AIDA. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating AIDA:', error);
      toast.error('Error generating AIDA. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedAIDA.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedAIDA]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedAIDA.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "AIDA.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'AIDA',
      text: generatedAIDA.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing AIDA:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Company/Product Name</label>
        <input
          value={companyName}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedAIDA.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedAIDA.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated AIDA</h1>
