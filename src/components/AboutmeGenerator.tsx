...
export function AboutmeGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/aboutme?clerkId=${userId}`, {
        personalInfo,
        outputLength,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedAboutme(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedAboutme.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedAboutme]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedAboutme.join("\n\n")], { type: "text/plain" });
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">personalInfo</label>
        <input
          type="text"
          value={personalInfo}
          onChange={(e) => setpersonalInfo(e.target.value)}
          placeholder="E.g., John is a tech enthusiast with 5 years of experience in video editing"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">output Length</label>
        <select
          value={outputLength}
          onChange={(e) => setoutputLength(e.target.value)}
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        >
          {outputLengths.map((lengthOption) => (
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedAboutme.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated About me</h1>
              ...
              {generatedAboutme.map((post, index) => (
