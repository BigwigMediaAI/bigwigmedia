...
export function ArticleConclusion() {
  ...
  const [generatedArticleConclusion, setgeneratedArticleConclusion] = useState([]);
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateArticleConclusion?clerkId=${userId}`, {
        title,
        keyPoints,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedArticleConclusion(response.data.conclusions);
      } else {
        toast.error('Error generating article conclusion. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedArticleConclusion.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedArticleConclusion]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedArticleConclusion.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Article_Conclusion.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Article conclusion',
      text: generatedArticleConclusion.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing article conclusion:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Topic of the Article</label>
        <input
          type="text"
          value={title}
          onChange={(e) => settitle(e.target.value)}
          placeholder="E.g., The importance of renewable energy sources in combating climate change"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter The Key Points</label>
        <textarea
          value={keyPoints}
          onChange={(e) => setkeyPoints(e.target.value)}
          placeholder="E.g., Benefits of renewable energy,Environmental impact,Economic advantages,Future potential"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
          rows={4}
        />
      </div>
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedArticleConclusion.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
...
          </div>
            ...
            generatedArticleConclusion.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Article Conclusion</h1>
              ...
              {generatedArticleConclusion.map((post, index) => (
