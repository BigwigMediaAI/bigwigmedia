...
export function GenerateArticleIntro() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/GenerateArticleIntro?clerkId=${userId}`, {
        topic,
        tone,
        language,
        outputCount
      ...
      if (response.status === 200) {
        ...
        setgeneratedArticleIntro(response.data);
      } else {
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    element.download = "ArticleIntro.txt";
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Give a topic to generate article intro</label>
        <textarea
          value={topic}
          onChange={(e) => setTopic(e.target.value)}
          placeholder="Enter the topic or main idea (e.g., The Future of Renewable Energy)"
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Article Intro</h1>
