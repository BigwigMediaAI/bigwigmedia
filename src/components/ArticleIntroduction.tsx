...
export function ArticleIntro() {
  ...
  const [generatedArticleIntro, setgeneratedArticleIntro] = useState([]);
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateArticleIntroduction?clerkId=${userId}`, {
        title,
        keyPoints,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedArticleIntro(response.data.introductions);
      } else {
        toast.error('Error generating article introduction. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedArticleIntro.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedArticleIntro]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedArticleIntro.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Article_Intro.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedArticleIntro.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing article introduction:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedArticleIntro.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
...
          </div>
            ...
            generatedArticleIntro.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Article Inroduction</h1>
              ...
              {generatedArticleIntro.map((post, index) => (
