...
export function GenerateArticleOutline() {
  ...
  const [generatedArticleOutline, setgeneratedArticleOutline] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/GenerateArticleOutline?clerkId=${userId}`, {
        topic,
        tone,
        language,
        outputCount
      ...
      if (response.status === 200) {
        ...
        setgeneratedArticleOutline(response.data);
      } else {
        toast.error('Error generating article outline. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedArticleOutline.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedArticleOutline]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedArticleOutline.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Article Idea',
      text: generatedArticleOutline.join("\n\n"),
    };
    try {
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Give a topic to generate article outline</label>
        <textarea
          value={topic}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedArticleOutline.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedArticleOutline.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Article Outline</h1>
              ...
              {generatedArticleOutline.map((post, index) => (
