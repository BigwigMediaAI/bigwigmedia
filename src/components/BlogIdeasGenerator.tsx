...
export function GenerateBlogIdeas() {
  ...
  const [generatedBlogIdea, setgeneratedBlogIdea] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/GenerateBlogIdeas?clerkId=${userId}`, {
        topic,
        language,
        outputCount
      });
      ...
      if (response.status === 200) {
        ...
        setgeneratedBlogIdea(response.data);
      } else {
        toast.error('Error generating idea. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedBlogIdea.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedBlogIdea]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBlogIdea.join("\n\n")], { type: "text/plain" });
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Give a topic to generate blog ideas</label>
        <textarea
          value={topic}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBlogIdea.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedBlogIdea.map((post, index) => (
