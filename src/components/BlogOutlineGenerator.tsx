...
export function GenerateBlogOutline() {
  ...
  const [generatedBlogOutline, setgeneratedBlogOutline] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/GenerateBlogOutline?clerkId=${userId}`, {
        topic,
        mainPointsForIntro,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedBlogOutline(response.data);
      } else {
        toast.error('Error generating outlines. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedBlogOutline.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedBlogOutline]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBlogOutline.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Blog_outlines.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Blog outlines',
      text: generatedBlogOutline.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing outlines:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Give a topic to generate blog outlines</label>
        <textarea
          value={topic}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBlogOutline.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedBlogOutline.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Blog Outline</h1>
              ...
              {generatedBlogOutline.map((post, index) => (
