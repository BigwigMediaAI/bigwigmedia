...
export function BlogPostConclusion() {
  ...
  const [generatedBlogPostConclusion, setgeneratedBlogPostConclusion] = useState([]);
...
  };
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setgeneratedBlogPostConclusion([]);
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateBlogPostConclusion?clerkId=${userId}`, {
        title,
        mainPoints,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedBlogPostConclusion(response.data.conclusions);
      } else {
        toast.error('Error generating blog post conclusion. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedBlogPostConclusion.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedBlogPostConclusion]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBlogPostConclusion.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "BlogPost_Conclusion.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedBlogPostConclusion.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing blog post conclusion:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter The Main Points</label>
        <textarea
          value={mainPoints}
          onChange={(e) => setmainPoints(e.target.value)}
          placeholder="E.g., Advancements in AI, Ethical Implications, Impact on Jobs"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
          rows={4}
        />
      </div>
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g., Tech enthusiasts, AI researchers, and general readers interested in technology"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBlogPostConclusion.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
...
          </div>
            ...
            generatedBlogPostConclusion.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Blog Post Conclusion</h1>
              ...
              {generatedBlogPostConclusion.map((post, index) => (
