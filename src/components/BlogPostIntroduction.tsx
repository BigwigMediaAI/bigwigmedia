...
export function BlogPostIntro() {
  ...
  const [generatedBlogPostIntro, setgeneratedBlogPostIntro] = useState([]);
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateBlogIntroduction?clerkId=${userId}`, {
        title,
        mainPoints,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedBlogPostIntro(response.data.introductions);
      } else {
        toast.error('Error generating blog post introduction. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating blog post introduction:', error);
      toast.error('Error generating blog post introduction. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedBlogPostIntro.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedBlogPostIntro]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBlogPostIntro.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "BlogPost_Intro.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedBlogPostIntro.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing blog post introduction:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBlogPostIntro.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
...
          </div>
            ...
            generatedBlogPostIntro.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Blog Post Inroduction</h1>
              ...
              {generatedBlogPostIntro.map((post, index) => (
