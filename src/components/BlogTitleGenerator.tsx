...
export function GenerateBlogTitle() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/GenerateBlogTitles?clerkId=${userId}`, {
        topic,
        tone,
        language,
        outputCount
      ...
      if (response.status === 200) {
        ...
        setgeneratedBlogTitle(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedBlogTitle.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedBlogTitle]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBlogTitle.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Blog_titles.txt";
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Give a topic to generate blog titles</label>
        <textarea
          value={topic}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBlogTitle.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedBlogTitle.map((post, index) => (
