...
export function BulletPointGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateBulletPoints?clerkId=${userId}`, {
        topic,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedBulletPoints(response.data);
      } else {
        toast.error('Error generating bullet points. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedBulletPoints.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedBulletPoints]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBulletPoints.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "bullet_point.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'bullet point',
      text: generatedBulletPoints.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing bullet points:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBulletPoints.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className=" border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedBulletPoints.map((post, index) => (
