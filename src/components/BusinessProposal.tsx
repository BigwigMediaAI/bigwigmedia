...
export function BusinessProposal() {
  ...
  const [generatedBusinessProposal, setgeneratedBusinessProposal] = useState([]);
  ...
  const handleGenerate = async () => {
    if (
      ...
      !validateInput(objectives)||
      !validateInput(deliverables)
    ) {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateBusinessProposal?clerkId=${userId}`, {
        companyName,
        projectDescription,
        objectives: objectives.split(',').map(objective => objective.trim()),
        deliverables:deliverables.split(',').map(deliverable => deliverable.trim()),
        timeline,
        budget,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedBusinessProposal(response.data);
      } else {
        toast.error('Error generating business proposal. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedBusinessProposal.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedBusinessProposal]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBusinessProposal.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "business_proposal.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedBusinessProposal.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing business proposal:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Project Description</label>
        <input
          value={projectDescription}
          onChange={(e) => setprojectDescription(e.target.value)}
          placeholder='E.g, Development of a new AI-based analytics platform'
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Objectives</label>
        <input
          value={objectives}
          onChange={(e) => setobjectives(e.target.value)}
          placeholder="E.g, Increase data processing speed, Improve data accuracy, Enhance user experience"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Deliverables</label>
        <input
          value={deliverables}
          onChange={(e) => setdeliverables(e.target.value)}
          placeholder="E.g, AI algorithm, User interface, Documentation"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Timeline</label>
        <input
          value={timeline}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Budget</label>
        <input
          value={budget}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBusinessProposal.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedBusinessProposal.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
              ...
              {generatedBusinessProposal.map((post, index) => (
