...
export function CTAGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateCallToAction?clerkId=${userId}`, {
        targetAudience,
        purpose,
        desiredAction,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedCTA(response.data);
      } else {
        toast.error('Error generating cta. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedCTA.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedCTA]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedCTA.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "cta.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'cta',
      text: generatedCTA.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing cta:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Purpose</label>
        <input
          value={purpose}
          onChange={(e) => setpurpose(e.target.value)}
          placeholder="E.g, signing up for a newsletter"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Desired Action</label>
        <input
          value={desiredAction}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedCTA.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
