...
export function CatchtTagline() {
  ...
  const [catchyTagline, setcatchyTagline] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateCatchyTagline?clerkId=${userId}`, {
        productDescription,
        targetAudience,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setcatchyTagline(response.data);
      } else {
        toast.error('Error generating catchy tagline. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && catchyTagline.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, catchyTagline]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([catchyTagline.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "catchy_tagline.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: catchyTagline.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing catchy tagline:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        ...
        <textarea
          ...
          placeholder="E.g, A revolutionary new smartphone with cutting-edge features and sleek design"
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder='tech-savvy individuals'
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (catchyTagline.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            catchyTagline.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated taglines</h1>
