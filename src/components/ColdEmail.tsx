...
export function ColdEmail() {
  ...
  const handleGenerate = async () => {
    if (
      ...
      !validateInput(yourCompanyName)||
      !validateInput(yourProductService)
    ) {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateColdEmail?clerkId=${userId}`, {
        recipientName,
        companyName,
        yourCompanyName,
        yourProductService,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating email. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedEmail.length>0) {
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedEmail.join("\n\n")], { type: "text/plain" });
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Reciepient Company Name</label>
        <input
          value={companyName}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Your Company Name</label>
        <input
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Your Product Service</label>
        <input
          value={yourProductService}
          onChange={(e) => setyourProductService(e.target.value)}
          placeholder="E.g, AI-powered natural language processing tools"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedEmail.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
