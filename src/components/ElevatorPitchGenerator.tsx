...
export function ElevatorPitch() {
  ...
  const handleGenerate = async () => {
    if (
      ...
      !validateInput(uniqueSellingPoint)
    ) {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateElevatorPitch?clerkId=${userId}`, {
        name,
        profession,
        targetAudience,
        uniqueSellingPoint,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating elevator pitch. Please try again later.');
      }
    } catch (error) {
  ...
  const handleDownload = () => {
    ...
    element.download = "elevator_pitch.txt";
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('Error sharing elevator pitch:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Unique Selling Point</label>
        <input
          value={uniqueSellingPoint}
          onChange={(e) => setuniqueSellingPoint(e.target.value)}
          placeholder="E.g, increasing brand visibility through innovative digital campaigns"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
