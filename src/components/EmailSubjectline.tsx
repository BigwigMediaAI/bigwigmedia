...
export function EmailSubjectLine() {
  ...
  const [generatedEmailSubjects, setgeneratedEmailSubjects] = useState([]);
  ...
  const handleGenerate = async () => {
    if (
      !validateInput(emailPurpose)
    ) {
      toast.error('Your input contains prohibited words. Please remove them and try again.');
      return;
    }
    setIsLoading(true);
    setgeneratedEmailSubjects([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateEmailSubjectLine?clerkId=${userId}`, {
        emailPurpose,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedEmailSubjects(response.data);
      } else {
        toast.error('Error generating email subjects. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedEmailSubjects.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedEmailSubjects]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedEmailSubjects.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "emai_subject.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Email Subject',
      text: generatedEmailSubjects.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing email subjects:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Email Purpose</label>
        <input
          value={emailPurpose}
          onChange={(e) => setemailPurpose(e.target.value)}
          placeholder="E.g, Announcing a new product launch"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedEmailSubjects.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedEmailSubjects.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
              ...
              {generatedEmailSubjects.map((post, index) => (
