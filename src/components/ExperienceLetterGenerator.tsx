...
export function ExperienceLetter() {
  ...
  const [generatedExperinceLetter, setgeneratedExperinceLetter] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateExperienceLetter?clerkId=${userId}`, {
        employeeName,
        position,
        department,
        duration,
      ...
      if (response.status === 200) {
        ...
        setgeneratedExperinceLetter(response.data);
      } else {
        toast.error('Error generating experience letter. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedExperinceLetter.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedExperinceLetter]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedExperinceLetter.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "experience_letter.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedExperinceLetter.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing experience letter:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Employee Name</label>
        <input
          value={employeeName}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedExperinceLetter.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedExperinceLetter.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
              ...
              {generatedExperinceLetter.map((post, index) => (
