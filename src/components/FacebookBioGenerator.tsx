...
export function GenerateFacebookBio() {
  ...
  const [generatedFbBio, setgeneratedFbBio] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateFacebookBio?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedFbBio(response.data);
      } else {
        toast.error('Error generating bios. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedFbBio.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedFbBio]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedFbBio.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "bio.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Facebook bios',
      text: generatedFbBio.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing bios:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s Your Facebook Bio About?</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="Please copy and paste or write your Facebook bio description. 
(e.g., Passionate traveler and foodie. Love exploring new cultures and trying new recipes.)"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedFbBio.length > 0 ? "Regenerate" : 'Generate')}
        </button>
