...
export function GenerateFacebookGroupPost() {
  ...
  const [generatedFbGroupPost, setgeneratedFbGroupPost] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateFacebookGroupPost?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedFbGroupPost(response.data.posts);
  ...
  useEffect(() => {
    if (!isLoading && generatedFbGroupPost.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedFbGroupPost]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedFbGroupPost.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedFbGroupPost.join("\n\n"),
    };
    try {
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s Your Facebook Group Post About?</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="Please copy and paste or write the details of your Facebook group post. 
(e.g., Looking for recommendations on the best local restaurants)."
    ...
    <label className="text-lg font-medium text-gray-700">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedFbGroupPost.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            generatedFbGroupPost.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Facebook Group Post</h1>
              ...
              {generatedFbGroupPost.map((post, index) => (
