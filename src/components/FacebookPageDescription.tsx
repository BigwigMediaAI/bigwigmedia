...
export function GenerateFacebookPageDescription() {
  ...
  const [generatedFbPageDescription, setgeneratedFbPageDescription] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateFacebookPageDescription?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedFbPageDescription(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedFbPageDescription.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedFbPageDescription]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedFbPageDescription.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedFbPageDescription.join("\n\n"),
    };
    try {
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s Your Video About?</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="Please describe your Facebook page.
(e.g.,This page shares tips and tutorials for DIY home projects.)"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedFbPageDescription.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            generatedFbPageDescription.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Facebook Page Description</h1>
              ...
              {generatedFbPageDescription.map((post, index) => (
