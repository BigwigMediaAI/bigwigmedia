...
export function FollowupEmail() {
  ...
  const [purposeOfFollowUpEmail, setpurposeOfFollowUpEmail] = useState('');
  ...
  const handleGenerate = async () => {
    if (
      !validateInput(mailReceived)||
      !validateInput(purposeOfFollowUpEmail)
    ) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setgeneratedReplies(response.data);
      } else {
        toast.error('Error generating follow up email. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedReplies.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedReplies]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedReplies.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Followup_email.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedReplies.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing follow up email:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">mailReceived</label>
        <input
          type="text"
          value={mailReceived}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Purpose of Follow Up Email</label>
        <input
          type="text"
          value={purposeOfFollowUpEmail}
          onChange={(e) => setpurposeOfFollowUpEmail(e.target.value)}
          placeholder="E.g., To discuss the next steps and project timelines"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedReplies.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Email</h1>
              ...
              {generatedReplies.map((post, index) => (
