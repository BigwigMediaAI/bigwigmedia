...
export function GMBProductDescription() {
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateGMBProductDescription?clerkId=${userId}`, {
        productName,
        productFeatures,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedProductDescription(response.data);
      } else {
        toast.error('Error generating product description. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedProductDescription.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedProductDescription]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedProductDescription.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "product_description.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedProductDescription.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing product description:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Product Features</label>
        <input
          value={productFeatures}
          onChange={(e) => setproductFeatures(e.target.value)}
          placeholder="E.g, Durable, user-friendly, affordable"
          className="mt-1 block w-full rounded-md  border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4 bg-white text-[var(--primary-text-color)]"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedProductDescription.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedProductDescription.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Product Description</h1>
              ...
              {generatedProductDescription.map((post, index) => (
