...
export function GenerateGoogleAdDescription() {
  ...
  const [generatedGoogleAdDescriptions, setgeneratedGoogleAdDescriptions] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setgeneratedGoogleAdDescriptions([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateGoogleAdDescription?clerkId=${userId}`, {
        serviceName,
      ...
      if (response.status === 200) {
        ...
        setgeneratedGoogleAdDescriptions(response.data);
      } else {
        toast.error('Error generating google ad descriptions. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating google ad descriptions:', error);
      toast.error('Error generating google ad descriptions. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedGoogleAdDescriptions.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedGoogleAdDescriptions]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedGoogleAdDescriptions.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "google_ad_descriptions.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedGoogleAdDescriptions.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing google ad descriptions:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Important Keywords</label>
        <textarea
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedGoogleAdDescriptions.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            generatedGoogleAdDescriptions.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Google Ad Description</h1>
              ...
              {generatedGoogleAdDescriptions.map((post, index) => (
