...
  };
  ...
  const handleGenerate = async () => {
    if (
      !validateInput(policyType)||
      !validateInput(companyName)||
      !validateInput(industry)||
      !validateInput(policyDetails)
    ) {
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateHRPolicy?clerkId=${userId}`, {
        policyType,
        companyName,
        industry,
        policyDetails,
      ...
      if (response.status === 200) {
        ...
        setgeneratedPolicy(response.data);
      } else {
        toast.error('Error generating HR Policy. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedPolicy.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedPolicy]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedPolicy.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'HR Policy',
      text: generatedPolicy.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing HR Policy:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
  <label className="block text-[var(--primary-text-color)]">Select Policy Type</label>
  <select
    value={policyType}
    onChange={(e) => setpolicyType(e.target.value)}
    className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm p-3 mb-4 bg-white"
  >
    <option value="" disabled>Select a policy type</option>
    <option value="Work-from-Home Policy">Work-from-Home Policy</option>
    <option value="Leave Policy">Leave Policy</option>
    <option value="Code of Conduct">Code of Conduct</option>
    <option value="Attendance Policy">Attendance Policy</option>
    <option value="Salary & Benefits Policy">Salary & Benefits Policy</option>
    <option value="Equal Opportunity Policy">Equal Opportunity Policy</option>
  </select>
</div>

      
      <div className="mb-5">
  <label className="block text-[var(--primary-text-color)]">Select Industry</label>
  <select
    value={industry}
  ...
  >
    ...
    <option value="Hospitality">Hospitality</option>
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Policy Details</label>
        <textarea
          rows={4}
          value={policyDetails}
          onChange={(e) => setpolicyDetails(e.target.value)}
          placeholder="E.g, Employees can work remotely up to 3 days a week, must be available for virtual meetings, and submit weekly reports."
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedPolicy.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Policy</h1>
              ...
              {generatedPolicy.map((post, index) => (
