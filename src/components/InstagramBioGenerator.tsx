...
export function GenerateInstagramBio() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateInstagramBio?clerkId=${userId}`, {
        profile,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedBios(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedBios.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedBios]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBios.join("\n\n")], { type: "text/plain" });
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s Your Instagram Profile About?</label>
        <textarea
          value={profile}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBios.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Bio</h1>
