...
export function InstagramHashtag() {
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateInstagramHashtags?clerkId=${userId}`, {
        topic,
        language,
        outputCount,
      });
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating instagram hashtag. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating instagram hashtag:', error);
      toast.error('Error generating instagram hashtag. Please try again later.');
    } finally {
      setIsLoading(false);
    }
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    element.download = "instagram_hashtag.txt";
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('Error sharing instagram hashtag:', err);
    }
  };
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Instagram Hashtags</h1>
