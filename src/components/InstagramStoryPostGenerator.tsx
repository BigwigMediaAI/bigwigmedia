...
export function GenerateInstagramStoryPost() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateInstagramStory?clerkId=${userId}`, {
        story,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedStory(response.data.posts);
  ...
  useEffect(() => {
    if (!isLoading && generatedStory.length > 0) {
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedStory.join("\n\n")], { type: "text/plain" });
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What is your story related to</label>
        <textarea
          value={story}
          onChange={(e) => setstory(e.target.value)}
          placeholder="Please copy and paste or write your Instagram story is about. 
(e.g.,Exciting announcement: New product launch tomorrow! Stay tuned!)"
    ...
    <label className="text-lg font-medium text-gray-700">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedStory.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Story Post</h1>
