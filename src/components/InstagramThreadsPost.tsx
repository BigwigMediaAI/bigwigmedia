...
export function GenerateInstagramThreadsPost() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateThreadsPost?clerkId=${userId}`, {
        theme,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedThreads(response.data.posts);
        setGeneratedImageUrl(response.data.imageUrl);
      } else {
        toast.error('Error generating threads. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedThreads.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedThreads]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedThreads.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "threads.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedThreads.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing threads:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s Your Instagram Thread About?</label>
        <textarea
          value={theme}
          onChange={(e) => settheme(e.target.value)}
          placeholder="Please copy and paste or write what your Instagram Thread is about.
(e.g. Mindfulness and Mental Health.)"
    ...
    <label className="text-lg font-medium text-gray-700">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedThreads.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Thread Post</h1>
              ...
              {generatedThreads.map((post, index) => (
