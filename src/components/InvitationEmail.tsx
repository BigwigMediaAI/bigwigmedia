...
export function EventInvitation() {
  ...
  const handleGenerate = async () => {
    if (
      ...
      !validateInput(eventDescription)||
      !validateInput(eventLocation)||
      !validateInput(recipientName)||
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateEventInvitationEmail?clerkId=${userId}`, {
        eventName,
        eventDate,
        eventLocation,
        eventDescription,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating event invitation. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating event invitation:', error);
      toast.error('Error generating event invitation. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedInvitation.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedInvitation]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedInvitation.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedInvitation.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing event invitation:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Event Name</label>
        <input
          type="text"
          value={eventName}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Event Date</label>
        <input
          type="Date"
          value={eventDate}
          onChange={(e) => seteventDate(e.target.value)}
          placeholder="E.g, 2024-09-25"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)]  shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Event Location</label>
        <input
          type="text"
          value={eventLocation}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Event Description</label>
        <input
          type="text"
          value={eventDescription}
          onChange={(e) => seteventDescription(e.target.value)}
          placeholder="E.g, Join us for a day of insightful talks and networking with industry leaders."
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)]  shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Recipient Name</label>
        <input
          type="text"
          value={recipientName}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Sender Name</label>
        <input
          type="text"
          value={senderName}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedInvitation.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)]  rounded-md mt-6 p-5 relative">
              ...
              {generatedInvitation.map((post, index) => (
