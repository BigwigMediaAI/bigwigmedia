...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateJobOffer?clerkId=${userId}`, {
        recipientName,
        position,
        companyName,
        startDate,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating offer letter. Please try again later.');
      }
    } catch (error) {
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    element.download = "offer_letter.txt";
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('Error sharing offer letter:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Recipient Name</label>
        <input
          value={recipientName}
