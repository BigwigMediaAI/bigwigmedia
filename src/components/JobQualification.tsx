...
export function JobQualification() {
  ...
  const [generatedQualificaition, setgeneratedQualificaition] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateJobQualifications?clerkId=${userId}`, {
        jobTitle,
        companyName,
        responsibilities: responsibilities.split(',').map(keyword => keyword.trim()),
        tone,
        language,
        outputCount,
      });
      ...
      if (response.status === 200) {
        ...
        setgeneratedQualificaition(response.data);
      } else {
        toast.error('Error generating qualification. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedQualificaition.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedQualificaition]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedQualificaition.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "qualification.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'qualification',
      text: generatedQualificaition.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing qualification:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Responsibilities</label>
        <input
          value={responsibilities}
          onChange={(e) => setresponsibilities(e.target.value)}
          placeholder="E.g, Develop and maintain web applications, Collaborate with cross-functional teams, Write clean, scalable code"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedQualificaition.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedQualificaition.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
              ...
              {generatedQualificaition.map((post, index) => (
