...
export function JobResponsibility() {
  ...
  const [generatedJobResponsibility, setgeneratedJobResponsibility] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateJobResponsibilities?clerkId=${userId}`, {
        companyName,
        jobTitle,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedJobResponsibility(response.data);
      } else {
        toast.error('Error generating job responsibility. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating job responsibility:', error);
      toast.error('Error generating job responsibility. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedJobResponsibility.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedJobResponsibility]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedJobResponsibility.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "job.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedJobResponsibility.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing job responsibility:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedJobResponsibility.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedJobResponsibility.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
              ...
              {generatedJobResponsibility.map((post, index) => (
