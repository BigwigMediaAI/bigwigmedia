...
  };
  ...
  const handleGenerate = async () => {
    if (
      ...
      !validateInput(keyResponsibilities)||
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateJobSummary?clerkId=${userId}`, {
        jobTitle,
        companyName,
        keyResponsibilities: keyResponsibilities.split(',').map(keyResponsibility => keyResponsibility.trim()),
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Key Responsibilities</label>
        <textarea
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Requirements</label>
        <textarea
          value={requirements}
          onChange={(e) => setRequirements(e.target.value)}
          placeholder="E.g, Bachelor's degree in Computer Science, 3+ years of experience in software development, Proficiency in JavaScript and React"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Location</label>
        <input
          value={location}
