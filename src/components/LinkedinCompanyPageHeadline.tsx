...
export function GenerateLinkedinCompanyPageHeadline() {
  ...
  const [generatedLinkedinCompanyPageHeadline, setgeneratedLinkedinCompanyPageHeadline] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setgeneratedLinkedinCompanyPageHeadline([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateLinkedinCompanyPageHeadline?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedLinkedinCompanyPageHeadline(response.data);
      } else {
        toast.error('Error generating headlines. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedLinkedinCompanyPageHeadline.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedLinkedinCompanyPageHeadline]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedLinkedinCompanyPageHeadline.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedLinkedinCompanyPageHeadline.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing headlines:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter your company headline</label>
        <textarea
          value={description}
...
(e.g., Innovating the future of clean energy with cutting-edge technology and sustainable practices)"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedLinkedinCompanyPageHeadline.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            generatedLinkedinCompanyPageHeadline.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Linkedin Company Page Headline</h1>
              ...
              {generatedLinkedinCompanyPageHeadline.map((post, index) => (
