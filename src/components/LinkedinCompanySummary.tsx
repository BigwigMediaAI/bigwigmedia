...
export function GenerateLinkedinCompanySummary() {
  ...
  const [generatedLinkedinCompanySummary, setgeneratedLinkedinCompanySummary] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setgeneratedLinkedinCompanySummary([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateLinkedinCompanySummary?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedLinkedinCompanySummary(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedLinkedinCompanySummary.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedLinkedinCompanySummary]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedLinkedinCompanySummary.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedLinkedinCompanySummary.join("\n\n"),
    };
    try {
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Describe Your Company</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="e.g. Empowering businesses with cutting-edge technology solutions | Your partner in digital transformation"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedLinkedinCompanySummary.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedLinkedinCompanySummary.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Linkedin Company Summary</h1>
              ...
              {generatedLinkedinCompanySummary.map((post, index) => (
