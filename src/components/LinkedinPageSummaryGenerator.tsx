...
export function GenerateLinkedinPageSummary() {
  ...
  const [generatedLinkedinPageSummary, setgeneratedLinkedinPageSummary] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setgeneratedLinkedinPageSummary([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateLinkedinPageSummary?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedLinkedinPageSummary(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedLinkedinPageSummary.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedLinkedinPageSummary]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedLinkedinPageSummary.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedLinkedinPageSummary.join("\n\n"),
    };
    try {
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedLinkedinPageSummary.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedLinkedinPageSummary.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Linkedin Page Summary</h1>
              ...
              {generatedLinkedinPageSummary.map((post, index) => (
