...
export function GenerateMarketingFunnel() {
  ...
  const [generatedMarketingFunnel, setgeneratedMarketingFunnel] = useState([]);
  ...
  const handleGenerate = async () => {
    if (!validateInput(productOrService)) {
      toast.error('Your input contains prohibited words. Please remove them and try again.');
      return;
    }
    setIsLoading(true);
    setgeneratedMarketingFunnel([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateMarketingFunnel?clerkId=${userId}`, {
        productOrService,
        goal,
        targetAudience,
        budget,
      ...
      if (response.status === 200) {
        ...
        setgeneratedMarketingFunnel(response.data);
      } else {
        toast.error('Error generating marketing funnel. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedMarketingFunnel.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedMarketingFunnel]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedMarketingFunnel.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "marketing_funnel.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedMarketingFunnel.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing marketing funnel:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What is your product or service</label>
        <input
          value={productOrService}
          onChange={(e) => setproductOrService(e.target.value)}
          placeholder="e.g. Premium Online Course on Digital Marketing"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
        />
      </div>

      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What is your target audience</label>
        <input
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What is your budget</label>
        <input
          value={budget}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Your Goal</label>
        <input
          value={goal}
          onChange={(e) => setgoal(e.target.value)}
          placeholder="e.g. Generate leads and convert them into course enrollments"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
        />
      </div>
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedMarketingFunnel.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedMarketingFunnel.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Marketing Funnel</h1>
              ...
              {generatedMarketingFunnel.map((post, index) => (
