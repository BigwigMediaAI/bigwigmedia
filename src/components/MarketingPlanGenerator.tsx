...
export function GenerateMarketingPlan() {
  ...
  const [generatedMarketingPlan, setgeneratedMarketingPlan] = useState([]);
  ...
  const handleGenerate = async () => {
    if (!validateInput(describeBusiness)) {
      toast.error('Your input contains prohibited words. Please remove them and try again.');
      return;
    }
    setIsLoading(true);
    setgeneratedMarketingPlan([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateMarketingPlan?clerkId=${userId}`, {
        describeBusiness,
        goal,
        objective,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedMarketingPlan(response.data);
      } else {
        toast.error('Error generating marketing plan. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedMarketingPlan.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedMarketingPlan]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedMarketingPlan.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "marketing_plan.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedMarketingPlan.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing marketing plan:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Describe Your Business</label>
        <input
          value={describeBusiness}
          onChange={(e) => setdescribeBusiness(e.target.value)}
          placeholder="e.g. A tech startup specializing in AI-powered productivity tools."
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="e.g.Increase brand awareness and market share."
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
        />
      </div>

      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Your Objective</label>
        <input
          value={objective}
          onChange={(e) => setObjective(e.target.value)}
          placeholder="e.g. Launch a targeted social media campaign and collaborate with industry influencers."
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
        />
      </div>
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedMarketingPlan.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedMarketingPlan.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Marketing Plan</h1>
              ...
              {generatedMarketingPlan.map((post, index) => (
