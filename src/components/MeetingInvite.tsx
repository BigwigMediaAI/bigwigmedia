...
export function MeetingInvite() {
  ...
  const [generatedMeetingInvite, setgeneratedMeetingInvite] = useState([]);
...
  };
  ...
  const handleGenerate = async () => {
    if (
      !validateInput(meetingTitle)||
      !validateInput(agenda)
    ) {
      toast.error('Your input contains prohibited words. Please remove them and try again.');
      return;
    }
    setIsLoading(true);
    setgeneratedMeetingInvite([]);
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateMeetingInvite?clerkId=${userId}`, {
        meetingTitle,
        date,
        time,
      ...
      if (response.status === 200) {
        ...
        setgeneratedMeetingInvite(response.data);
      } else {
        toast.error('Error generating meeting invite. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedMeetingInvite.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedMeetingInvite]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedMeetingInvite.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "invite.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedMeetingInvite.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing meeting invite:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Meeting Title</label>
        <input
          value={meetingTitle}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Date</label>
        <input
          type='date'
          value={date}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Time</label>
        <input
          type='time'
          value={time}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Participants</label>
        <input
          value={participants}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Agenda</label>
        <input
          value={agenda}
          onChange={(e) => setagenda(e.target.value)}
          placeholder="E.g, Discuss project goals, timelines, and responsibilities"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedMeetingInvite.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedMeetingInvite.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedMeetingInvite.map((post, index) => (
