...
export function MetaDescription() {
  ...
  const handleGenerate = async () => {
    if (
      !validateInput(pageTitle)||
      !validateInput(pageContent)
    ) {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateMetaDescription?clerkId=${userId}`, {
        pageTitle,
        pageContent,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedmetaDescription(response.data);
      } else {
        toast.error('Error generating metaDescription. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating metaDescription:', error);
      toast.error('Error generating metaDescription. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedmetaDescription.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedmetaDescription]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedmetaDescription.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "metaDescription.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedmetaDescription.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing metaDescription:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Page Title</label>
        <input
          value={pageTitle}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Page Content</label>
        <textarea
          value={pageContent}
          onChange={(e) => setpageContent(e.target.value)}
          placeholder='E.g, Time management is crucial for success. In this article, we provide ten actionable tips to help you manage your time effectively...'
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedmetaDescription.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedmetaDescription.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Description</h1>
              ...
              {generatedmetaDescription.map((post, index) => (
