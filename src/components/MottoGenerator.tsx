...
export function MottoGenerator() {
  ...
  const [generatedMotto, setgeneratedMotto] = useState([]);
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateMotto?clerkId=${userId}`, {
        companyName,
        industry,
        values,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedMotto(response.data);
      } else {
        toast.error('Error generating motto. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedMotto.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedMotto]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedMotto.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "motto.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'motto',
      text: generatedMotto.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing motto:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Values</label>
        <input
          value={values}
          onChange={(e) => setvalues(e.target.value)}
          placeholder="E.g, Innovation, Excellence, Customer Satisfaction"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedMotto.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
