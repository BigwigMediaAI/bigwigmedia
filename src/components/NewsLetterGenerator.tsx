...
export function GenerateNewsLetter() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateNewsletter?clerkId=${userId}`, {
        organizationName,
        event,
        tone,
        language,
      ...
      console.error('Error generating news letter:', error);
      toast.error('Error generating news letter. Please try again later.');
    } finally {
      setIsLoading(false);
    }
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    element.download = "news_letter.txt";
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('Error sharing news letter:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        ...
        <textarea
          ...
          placeholder="e.g. Upcoming Webinar on AI Advancements"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
          rows={4}
        />
      </div>
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated News Letter</h1>
