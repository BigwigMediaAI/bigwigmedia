...
export function OkrGenerator() {
  ...
  const [keyResultsCount, setkeyResultsCount] = useState(1);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateOKR?clerkId=${userId}`, {
        objective,
        department,
        timeFrame,
        keyResultsCount,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating OKR. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedOkr.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedOkr]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedOkr.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "okr.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'OKR',
      text: generatedOkr.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing OKR:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Objective</label>
        <input
          value={objective}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Time Frame</label>
        <input
          value={timeFrame}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Key Results Count</label>
        <select
          value={keyResultsCount}
          onChange={(e) => setkeyResultsCount(Number(e.target.value))}
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        >
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedOkr.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
