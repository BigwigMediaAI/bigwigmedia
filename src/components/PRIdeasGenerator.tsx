...
export function PRideaGenerator() {
  ...
  const [generatedPRIdeas, setgeneratedPRIdeas] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generatePRIdeas`, {
        companyName,
        industry,
        targetAudience,
        goals,
      ...
      if (response.status === 200) {
        ...
        setgeneratedPRIdeas(response.data);
      } else {
        toast.error('Error generating PRIdea. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating PRIdea:', error);
      toast.error('Error generating PRIdea. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedPRIdeas.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedPRIdeas]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedPRIdeas.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "pridea.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'PRIdea',
      text: generatedPRIdeas.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing PRIdea:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g., EcoTech Innovations"
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g., Environmentally conscious tech enthusiasts"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g., Increase brand awareness, drive engagement, and establish thought leadership"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedPRIdeas.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
              ...
              {generatedPRIdeas.map((post, index) => (
