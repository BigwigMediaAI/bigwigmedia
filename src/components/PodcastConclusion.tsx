...
export function PodcastConclusion() {
  ...
  const [generatedPodcastConclusion, setgeneratedPodcastConclusion] = useState([]);
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generatePodcastConclusion?clerkId=${userId}`, {
        title,
        description,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedPodcastConclusion(response.data.conclusions);
      } else {
        toast.error('Error generating podcast conclusion. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedPodcastConclusion.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedPodcastConclusion]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedPodcastConclusion.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Podcast_Conclusion.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Podcast conclusion',
      text: generatedPodcastConclusion.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing podcast conclusion:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter The Description</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="E.g., Exploring how technology will shape our future lives and industries."
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
          rows={4}
        />
      </div>
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g., Tech enthusiasts and professionals"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedPodcastConclusion.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
...
          </div>
            ...
            generatedPodcastConclusion.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Podcast Conclusion</h1>
              ...
              {generatedPodcastConclusion.map((post, index) => (
