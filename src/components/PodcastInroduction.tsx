...
export function PodcastIntro() {
  ...
  const [generatedPodcastIntro, setgeneratedPodcastIntro] = useState([]);
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generatePodcastIntroduction?clerkId=${userId}`, {
        title,
        description,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedPodcastIntro(response.data.introductions);
      } else {
        toast.error('Error generating podcast introduction. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedPodcastIntro.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedPodcastIntro]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedPodcastIntro.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Podcast_Intro.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedPodcastIntro.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing podcast introduction:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedPodcastIntro.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
...
          </div>
            ...
            generatedPodcastIntro.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)] ">Generated Podcast Inroduction</h1>
              ...
              {generatedPodcastIntro.map((post, index) => (
