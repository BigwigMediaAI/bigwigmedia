...
  };
  ...
  const handleGenerate = async () => {
    if (
      !validateInput(podcastName) ||
      !validateInput(episodeTitle) ||
      !validateInput(episodeSummary)
    ) {
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/podcastNewsletter?clerkId=${userId}`, {
        podcastName,
        episodeTitle,
        episodeSummary,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedNewsletter(response.data.newsletters);
      } else {
        toast.error('Error generating newsletter. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedNewsletter.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedNewsletter]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedNewsletter.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "newsletter.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'newsletter',
      text: generatedNewsletter.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing newsletter:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Name of the Podcast</label>
        <input
          type="text"
          value={podcastName}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Episode Title</label>
        <input
          value={episodeTitle}
          onChange={(e) => setEpisodeTitle(e.target.value)}
          placeholder="E.g., AI Revolution: What's Next?"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Episode Summary</label>
        <textarea
          value={episodeSummary}
          rows={4}
          onChange={(e) => setepisodeSummary(e.target.value)}
          placeholder="E.g., In this episode, we discuss the latest developments in artificial intelligence and explore what the future holds."
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedNewsletter.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
