...
export function GeneratePostHashtag() {
  const [isLoading, setIsLoading] = useState(false);
  const [description, setdescription] = useState('');
  const [language, setLanguage] = useState('English');
  const [outputCount, setOutputCount] = useState(1);
  const [generatedPostHashtag, setgeneratedPostHashtag] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generatePostHashtags?clerkId=${userId}`, {
        description,
        language,
        outputCount
      });
      ...
      if (response.status === 200) {
        ...
        setgeneratedPostHashtag(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedPostHashtag.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedPostHashtag]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedPostHashtag.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "posthashtag.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'PostHashtag',
      text: generatedPostHashtag.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing hashtag:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Write keywords to generate Hashtags </label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="Enter relevant keywords related for your post"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedPostHashtag.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedPostHashtag.map((post, index) => (
