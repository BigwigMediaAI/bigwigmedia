...
export function GeneratePressRelease() {
  ...
  useEffect(() => {
    if (!isLoading && generatedPressRelease.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedPressRelease]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedPressRelease.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "press_release.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'press release',
      text: generatedPressRelease.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing press release:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedPressRelease.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Press Release</h1>
