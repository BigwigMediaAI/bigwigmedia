...
export function GeneratePrivacyPolicy() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setgeneratedPrivacyPolicy(response.data.data.improvedContent);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedPrivacyPolicy.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedPrivacyPolicy]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedPrivacyPolicy.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "privacy_ploicy.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedPrivacyPolicy.join("\n\n"),
    };
    try {
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedPrivacyPolicy.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedPrivacyPolicy.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedPrivacyPolicy.map((post, index) => (
