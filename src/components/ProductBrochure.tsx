...
export function ProductBrochure() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateProductBrochure?clerkId=${userId}`, {
        productName,
        productDescription,
        features,
        benefits,
      ...
      if (response.status === 200) {
        ...
        setgeneratedBrochure(response.data);
      } else {
        toast.error('Error generating brochure. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedBrochure.length>0) {
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedBrochure.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "brochure.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'brochure',
      text: generatedBrochure.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing brochure:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Product Description</label>
        <input
          value={productDescription}
          onChange={(e) => setproductDescription(e.target.value)}
          placeholder='E.g, A central hub for managing all your smart home devices.'
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Features</label>
        <input
          value={features}
          onChange={(e) => setfeatures(e.target.value)}
          placeholder="E.g, Voice control, Remote access, Energy saving, Easy setup"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Benefits</label>
        <input
          value={benefits}
          onChange={(e) => setBenefits(e.target.value)}
          placeholder="E.g, Convenience, Security, Energy efficiency, User-friendly"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedBrochure.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedBrochure.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Brochure</h1>
              ...
              {generatedBrochure.map((post, index) => (
