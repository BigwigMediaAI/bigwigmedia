...
export function ProductName() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateProductName?clerkId=${userId}`, {
        productDescription,
        targetAudience,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedProductName(response.data);
      } else {
        toast.error('Error generating product name. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedProductName.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedProductName]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedProductName.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "product_name.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'product name',
      text: generatedProductName.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing product name:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedProductName.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedProductName.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Product Name</h1>
              ...
              {generatedProductName.map((post, index) => (
