...
export function ProfessionalBioGenerator() {
  const [isLoading, setIsLoading] = useState(false);
  const [name, setname] = useState('');
  const [profession, setProfession] = useState('');
  const [achievements, setAachievements] = useState('');
  const [tone, setTone] = useState('');
  const [language, setLanguage] = useState('');
  const [outputCount, setOutputCount] = useState(1);
  const [generatedProfessionalBio, setgeneratedProfessionalBio] = useState([]);
  ...
  const handleGenerate = async () => {
    if (
      !validateInput(name)||
      !validateInput(profession)||
      !validateInput(achievements)
    ) {
      toast.error('Your input contains prohibited words. Please remove them and try again.');
      return;
    }
    setIsLoading(true);
    setgeneratedProfessionalBio([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateProfessionalBio?clerkId=${userId}`, {
        name,
        profession,
        achievements,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedProfessionalBio(response.data);
      } else {
        toast.error('Error generating professional Bio. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedProfessionalBio.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedProfessionalBio]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedProfessionalBio.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "professional_Bio.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedProfessionalBio.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing professional Bio:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Achievements</label>
        <input
          type="text"
          value={achievements}
          onChange={(e) => setAachievements(e.target.value)}
          placeholder="E.g, Developed several successful apps, Speaker at tech conferences"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedProfessionalBio.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedProfessionalBio.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Professional Bio</h1>
              ...
              {generatedProfessionalBio.map((post, index) => (
