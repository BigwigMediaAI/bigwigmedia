...
export function ProjectReport() {
  ...
  const handleGenerate = async () => {
    if (
      ...
      !validateInput(keyMilestones)||
      !validateInput(projectOutcome)
    ) {
      toast.error('Your input contains prohibited words. Please remove them and try again.');
      return;
    }
    setIsLoading(true);
    setgeneratedProjectReport([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateProjectReport?clerkId=${userId}`, {
        projectName,
        projectDescription,
        keyMilestones,
        projectOutcome,
      ...
      if (response.status === 200) {
        ...
        setgeneratedProjectReport(response.data);
      } else {
        toast.error('Error generating project report. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedProjectReport.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedProjectReport]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedProjectReport.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedProjectReport.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing project report:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Project Name</label>
        <input
          value={projectName}
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g, This project aims to develop a new feature for our application."
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Key Milestone</label>
        <input
          value={keyMilestones}
          onChange={(e) => setkeyMilestones(e.target.value)}
          placeholder="E.g, Design phase, Development phase, Testing phase, Launch phase"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Project Outcome</label>
        <input
          value={projectOutcome}
          onChange={(e) => setprojectOutcome(e.target.value)}
          placeholder="E.g, Successfully launched the new feature with positive feedback from users."
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedProjectReport.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedProjectReport.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Project Report</h1>
              ...
              {generatedProjectReport.map((post, index) => (
