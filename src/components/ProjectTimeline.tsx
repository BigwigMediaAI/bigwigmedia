...
export function ProjectTimeline() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateProjectTimeline?clerkId=${userId}`, {
        projectName,
        projectDescription,
        milestones: milestones.split(',').map(keyword => keyword.trim()),
        startDate,
        endDate,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedTimeline(response.data);
      } else {
        toast.error('Error generating timeline. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedTimeline.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedTimeline]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedTimeline.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "timeline.txt";
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        ...
        <textarea
          ...
          placeholder='E.g, A complete redesign of the company website to improve user experience and update the branding.'
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">milestones</label>
        <textarea
          value={milestones}
          onChange={(e) => setmilestones(e.target.value)}
          placeholder="E.g, Initial Planning, Wireframing, Design, Development, Testing, Launch"
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm  p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">End Date</label>
        <input
          type='date'
          value={endDate}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedTimeline.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedTimeline.map((post, index) => (
