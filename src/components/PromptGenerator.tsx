...
export function GeneratePrompt() {
  ...
  const [creativityLevel, setcreativityLevel] = useState('Moderate');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/GeneratePromptGenerator?clerkId=${userId}`, {
        context,
        purpose,
        creativityLevel,
        tone,
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('Error sharing prompts:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Describe Your Context</label>
        <textarea
          value={context}
          onChange={(e) => setcontext(e.target.value)}
          placeholder="e.g. Provide a brief description of the background or situation for which you need a prompt."
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="e.g. Specify the goal or intention behind the prompt (e.g., brainstorming, creative writing, inspiration)."
          className="mt-1 block w-full rounded-md border-1 border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
        />
      </div>

      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Creativity Level</label>
        <select
          value={creativityLevel}
          onChange={(e) => setcreativityLevel(e.target.value)}
          className="mt-1 block w-full rounded-md border border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
        >
          {creativityLevels.map((creativityLevelOption) => (
            <option key={creativityLevelOption.value} value={creativityLevelOption.value}>
              {creativityLevelOption.label}
            </option>
          ))}
        </select>
      </div>
