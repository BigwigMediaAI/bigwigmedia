...
export function RefrenceLetter() {
  ...
  const [generatedRefrenceLetter, setgeneratedRefrenceLetter] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateReferenceLetter?clerkId=${userId}`, {
        candidateName,
        relationship,
      ...
      if (response.status === 200) {
        ...
        setgeneratedRefrenceLetter(response.data);
      } else {
        toast.error('Error generating refrence letter. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating refrence letter:', error);
      toast.error('Error generating refrence letter. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedRefrenceLetter.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedRefrenceLetter]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedRefrenceLetter.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "refrence_letter.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'refrence letter',
      text: generatedRefrenceLetter.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing refrence letter:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Candidate Name</label>
        <input
          value={candidateName}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedRefrenceLetter.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedRefrenceLetter.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
              ...
              {generatedRefrenceLetter.map((post, index) => (
