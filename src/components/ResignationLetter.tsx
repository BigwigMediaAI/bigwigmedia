...
export function ResignationLetter() {
  ...
  const handleGenerate = async () => {
    if (
      ...
      !validateInput(lastWorkingDay)||
      !validateInput(reasonForLeaving)
    ) {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateResignationLetter?clerkId=${userId}`, {
        employeeName,
        position,
        companyName,
        lastWorkingDay,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating resignation letter. Please try again later.');
      }
    } catch (error) {
  ...
  const handleDownload = () => {
    ...
    element.download = "resignation_letter.txt";
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('Error sharing resignation letter:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Last Working day</label>
        <input
          type="date"
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Reason For Leaving</label>
        <input
          value={reasonForLeaving}
          onChange={(e) => setReasonForLeaving(e.target.value)}
          placeholder="E.g, Pursuing higher education opportunities"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
