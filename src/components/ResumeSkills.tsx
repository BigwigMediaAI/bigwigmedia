...
export function ResumeSkills() {
  ...
  const [generatedResumeSkills, setgeneratedResumeSkills] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateResumeSkills?clerkId=${userId}`, {
        profession,
        experienceLevel,
        industry,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedResumeSkills(response.data);
      } else {
        toast.error('Error generating resume skills. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedResumeSkills.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedResumeSkills]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedResumeSkills.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "skills.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Resume skills',
      text: generatedResumeSkills.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing resume skills:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Experience Level</label>
        <input
      ...
      <div className="mt-5 flex justify-center">
        <button
          className="text-white text-center font-outfit md:text-lg font-semibold flex relative text-base py-3 px-10 justify-center items-center gap-4 flex-shrink-0 rounded-full bg-[var(--teal-color)] disabled:opacity-60 hover:bg-[var(--teal-color)] w-fit mx-auto"
          onClick={handleGenerate}
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedResumeSkills.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedResumeSkills.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedResumeSkills.map((post, index) => (
