...
export function SOPGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateSOP?clerkId=${userId}`, {
        applicantName,
        background,
        goals,
        whyThisProgram,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating SOP. Please try again later.');
      }
    } catch (error) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.error('Error sharing SOP:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Applicant Name</label>
        <input
          value={applicantName}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Background</label>
        <textarea
          value={background}
          onChange={(e) => setbackground(e.target.value)}
          placeholder='E.g, Bachelors degree in Computer Science from XYZ University with a strong foundation in software development and AI.'
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Goals</label>
        <textarea
          value={goals}
          onChange={(e) => setgoals(e.target.value)}
          placeholder="E.g, To pursue a Master's degree in Artificial Intelligence and contribute to innovative AI research and development."
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Why This Program</label>
        <textarea
          value={whyThisProgram}
          onChange={(e) => setwhyThisProgram(e.target.value)}
          placeholder="E.g, XYZ University's AI program is renowned for its cutting-edge research, distinguished faculty, and state-of-the-art facilities."
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
