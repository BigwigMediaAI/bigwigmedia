...
export function SeoBriefGenerator() {
  ...
  const [generatedSeoBriefs, setgeneratedSeoBriefs] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateSeoBrief?clerkId=${userId}`, {
        topic,
        keywords,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedSeoBriefs(response.data);
      } else {
        toast.error('Error generating SEO Brief. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating SEO Brief:', error);
      toast.error('Error generating SEO Brief. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedSeoBriefs.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedSeoBriefs]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedSeoBriefs.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "SEO_Brief.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'SEO Brief',
      text: generatedSeoBriefs.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing SEO Brief:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g, The Importance of SEO in Digital Marketing"
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g, SEO, Digital Marketing, Search Engine Optimization"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedSeoBriefs.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedSeoBriefs.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Seo Brief</h1>
              ...
              {generatedSeoBriefs.map((post, index) => (
