...
export function SeoCompetitorAnalysis() {
  ...
  const handleGenerate = async () => {
    if (!validateInput(competitorUrls)) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setgeneratedAnalysis(response.data);
      } else {
        toast.error('Error analysis seo. Please try again later.');
      }
    } catch (error) {
      console.error('Error analysis seo:', error);
      toast.error('Error analysis seo. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedAnalysis.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedAnalysis]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedAnalysis.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Seo analysis',
      text: generatedAnalysis.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing seo analysis:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="e.g. www.bigwigmedia.ai"
    ...
    />
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedAnalysis.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated SEO Analysis</h1>
              ...
              {generatedAnalysis.map((post, index) => (
