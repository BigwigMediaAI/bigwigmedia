...
export function SeoOptimizer() {
  ...
  const [generatedSeoOptimized, setgeneratedSeoOptimized] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/optimizeSEO?clerkId=${userId}`, {
        content,
        language,
        outputCount
      });
      ...
      if (response.status === 200) {
        ...
        setgeneratedSeoOptimized(response.data);
      } else {
        toast.error('Error optimizing seo. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedSeoOptimized.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedSeoOptimized]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedSeoOptimized.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedSeoOptimized.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing optimized seo:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter the content</label>
        <textarea
          value={content}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedSeoOptimized.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedSeoOptimized.map((post, index) => (
