...
export function GenerateSeoAndDescription() {
  const [isLoading, setIsLoading] = useState(false);
  const [companyName, setcompanyName] = useState('');
  const [description, setDescription] = useState('');
  const [fewKeywords, setFewKeywords] = useState('');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/GenerateSEOTitleDescription?clerkId=${userId}`, {
        companyName,
        description,
        fewKeywords:fewKeywords.split(',').map(fewKeyword => fewKeyword.trim()),
        tone,
        language,
        outputCount
      });
      ...
      if (response.status === 200) {
        ...
        setgeneratedSeotitle(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedSeotitle.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedSeotitle]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedSeotitle.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Seo_title.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Seo titles',
      text: generatedSeotitle.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing titles:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter your company name</label>
        <input
          value={companyName}
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Brief description about the Company/Products</label>
        <textarea
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          placeholder="e.g. We specialize in cutting-edge technology solutions for businesses of all sizes."
          className="mt-1 block w-full rounded-md border-1 border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
          rows={4}
        />
      </div>
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Paste your current headline or write just few keywords</label>
        <input
          value={fewKeywords}
          onChange={(e) => setFewKeywords(e.target.value)}
          placeholder="e.g. technology solutions, business innovation, cutting-edge"
          className="mt-1 block w-full rounded-md border-1 border-[var(--primary-text-color)] shadow-sm p-3 mb-4"
        />
      </div>
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedSeotitle.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
              ...
              {generatedSeotitle.map((post, index) => (
