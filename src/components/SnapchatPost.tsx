...
export function GenerateSnapchatPost() {
...
    }
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateSnapchatPost?clerkId=${userId}`, {
        story,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating snapchat posts. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating snapchat posts:', error);
      toast.error('Error generating snapchat posts. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedPosts.length > 0) {
  ...
  const handleCopy = (postContent: string) => {
  ...
  const handleDownload = () => {
    ...
    element.download = "linkedin_posts.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Generated snapchat posts',
      text: generatedPosts.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing snapchat posts:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg  bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
        <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s the story of your post?</label>
        <input
          type="text"
          value={story}
          onChange={(e) => setstory(e.target.value)}
          placeholder="E.g., A fun weekend adventure in the mountains"
    ...
    generatedPosts.length > 0 && (
      <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative ">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl text-[var(--primary-text-color)]">Generated Snapchat Post</h1>
