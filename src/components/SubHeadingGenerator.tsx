...
export function SubHeading() {
  ...
  const [generatedSubheading, setgeneratedSubheading] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateSubheadings?clerkId=${userId}`, {
        heading,
        context,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        setgeneratedSubheading(response.data);
      } else {
        toast.error('Error generating subheading. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedSubheading.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedSubheading]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedSubheading.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "subheading.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'subheading',
      text: generatedSubheading.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing subheading:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Heading/Title</label>
        <input
          value={heading}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Content Description</label>
        <input
          value={context}
          onChange={(e) => setContext(e.target.value)}
          placeholder='E.g, Customer feedback for our new software product'
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedSubheading.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
              ...
              {generatedSubheading.map((post, index) => (
