...
export function TextInfographic() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        setInfographicText(res.data.infographicText);
        setIsLoading(false);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && infographicText) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, infographicText]);

  const customStyles = {
    control: (provided: any) => ({
      ...provided,
      backgroundColor: '#262626', // Match the dark background
      borderColor: 'gray', // Match the border color of Textarea
    ...
    multiValue: (provided: any) => ({
      ...provided,
      backgroundColor: '#333333', // Slightly different dark color for selected options
      color: 'white',
    }),
              ...
              <div ref={resultsRef} className="flex flex-col gap-2 mt-4">
                <div className="h-40 w-full rounded-md border-2 border-gray-300 dark:text-gray-200 text-gray-800 p-5 overflow-y-scroll relative">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    onClick={() => handleCopy(infographicText)}
                  >
