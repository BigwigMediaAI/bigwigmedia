...
export function TiktokCaption() {
  ...
  const [tiktokCaptions, settiktokCaptions] = useState([]);
  ...
  const handleGenerate = async () => {
    if (
      !validateInput(videoDescription)
    ) {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/tiktokcaption?clerkId=${userId}`, {
        videoDescription,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        settiktokCaptions(response.data);
      } else {
        toast.error('Error generating tiktok caption. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && tiktokCaptions.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, tiktokCaptions]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([tiktokCaptions.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "tiktok_Caption.txt";
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-[var(--white-color)] p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g.,Having the best time at the beach! 🌊🏖️ #SummerVibes"
