...
export function AttentionGrabbingTitleGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateTitle?clerkId=${userId}`, {
        topic,
        keywords,
        targetAudience,
        tone,
  ...
  useEffect(() => {
    if (!isLoading && generatedTitles.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedTitles]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedTitles.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "attention_grabbing_titles.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Generated Attention-Grabbing Titles',
      text: generatedTitles.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing attention-grabbing titles:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g., AI, machine learning, future technology"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedTitles.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Attention-Grabbing Titles</h1>
              ...
              {generatedTitles.map((post, index) => (
