...
export function TweetReplyGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateTweetReply?clerkId=${userId}`, {
        tweetContent,
        userHandle,
        tone,
        language,
      ...
      if (response.status === 200) {
        ...
        toast.error('Error generating tweet reply. Please try again later.');
      }
    } catch (error) {
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Tweet Content</label>
        <input
          type="text"
          value={tweetContent}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">User Handle</label>
        <input
          type="text"
          value={userHandle}
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                ...
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Tweet Reply</h1>
