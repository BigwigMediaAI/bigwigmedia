...
export function GenerateTwitterPost() {
  ...
  const [generatedTwitterPosts, setgeneratedTwitterPosts] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateTwitterPost?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedTwitterPosts(response.data.posts);
  ...
  useEffect(() => {
    if (!isLoading && generatedTwitterPosts.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedTwitterPosts]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedTwitterPosts.join("\n\n")], { type: "text/plain" });
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Twitter post',
      text: generatedTwitterPosts.join("\n\n"),
    };
    try {
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s Your Twitter Post About?</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="Write or paste the content of your X (Twitter) post .
(e.g., Excited to announce our new product launch tomorrow!)"
    ...
    <label className="text-lg font-medium text-gray-700">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedTwitterPosts.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            generatedTwitterPosts.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Twitter Post</h1>
              ...
              {generatedTwitterPosts.map((post, index) => (
