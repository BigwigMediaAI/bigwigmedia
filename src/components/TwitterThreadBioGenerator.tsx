...
export function GenerateTwitterThreadBio() {
  ...
  const [generatedTwitterThreadsBio, setgeneratedTwitterThreadsBio] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateTwitterThreadsBio?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedTwitterThreadsBio(response.data);
      } else {
        toast.error('Error generating thread bios. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating thread bios:', error);
      toast.error('Error generating thread bios. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedTwitterThreadsBio.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedTwitterThreadsBio]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedTwitterThreadsBio.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "thread bios.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Twitter thread bios',
      text: generatedTwitterThreadsBio.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing thread bios:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s Your Twitter (x) Threads Bio About?</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="Write or paste the content for your Twitter Threads bio 
(e.g.,Tech enthusiast sharing tips and updates on the latest gadgets and software.)"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedTwitterThreadsBio.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedTwitterThreadsBio.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Twitter Thread Bio</h1>
              ...
              {generatedTwitterThreadsBio.map((post, index) => (
