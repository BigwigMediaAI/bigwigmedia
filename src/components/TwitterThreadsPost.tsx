...
export function GenerateTwitterThreadPost() {
  ...
  const [generatedTwitterThreadsPost, setgeneratedTwitterThreadsPost] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setgeneratedTwitterThreadsPost([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateTwitterThreadsPost?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedTwitterThreadsPost(response.data);
      } else {
        toast.error('Error generating thread posts. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating thread posts:', error);
      toast.error('Error generating thread posts. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedTwitterThreadsPost.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedTwitterThreadsPost]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedTwitterThreadsPost.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "thread posts.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Twitter thread posts',
      text: generatedTwitterThreadsPost.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing thread posts:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Your Thread Content</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="e.g.Kicking off a new project today! 🚀 Let’s talk about the top 5 tips for efficient API design. #TechTalk #DevLifeTech enthusiast | Coffee lover ☕"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedTwitterThreadsPost.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedTwitterThreadsPost.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Twitter Thread Post</h1>
              ...
              {generatedTwitterThreadsPost.map((post, index) => (
