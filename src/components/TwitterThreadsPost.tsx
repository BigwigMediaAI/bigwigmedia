  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setgeneratedTwitterThreadsPost(response.data.posts);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What’s Your Twitter (x) Threads Post About?</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="Write or paste the content of your X (Twitter) thread .
(e.g., Thread: Tips for mastering time management in remote work.)"
