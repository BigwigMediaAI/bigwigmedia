...
export function UniqueValue() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateUVP?clerkId=${userId}`, {
        productName,
        targetAudience,
        keyBenefit,
        tone,
      ...
      if (response.status === 200) {
        ...
        setgeneratedUVP(response.data);
      } else {
        toast.error('Error generating UVP. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedUVP.length>0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedUVP]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedUVP.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "UVP.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'UVP',
      text: generatedUVP.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing UVP:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Product Name/Title</label>
        <input
          value={productName}
      ...
      <div className="mb-5">
        <label className="block text-gray-700 dark:text-gray-300">Key Benefits</label>
        <input
          value={keyBenefit}
          onChange={(e) => setKeyBenefit(e.target.value)}
          placeholder='E.g, Sustainable hydration solution'
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedUVP.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
