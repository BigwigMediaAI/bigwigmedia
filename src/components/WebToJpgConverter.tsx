...
export function WebpToJpg() {
  ...
  const [conversionType, setConversionType] = useState("jpg")
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Filter out only image files (jpg, png, svg, etc.)
    const newFiles = Array.from(files).filter((file) =>
      ["image/webp"].includes(file.type)
    );
    
    if (newFiles.length < files.length) {
      toast.error("Please select valid image formats.");
    }
  ...
  const webpToJpg = async () => {
    ...
    try {
      if (selectedFiles.length === 0) {
        toast.error("Please select webp images.");
        return;
      }
    ...
    setTimeout(() => {
      ...
      const response = await axios.post(`${BASE_URL}/response/webpToImages?clerkId=${userId}`, formData, {
        responseType: "blob",
      });
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (newFiles.length < files.length) {
      toast.error("Please select only webp format files.");
    }
    setSelectedFiles((prevFiles) => [...prevFiles, ...newFiles].slice(0, 10));
  };
