...
export function GenerateYoutubePostDescription() {
  ...
  const [generatedYoutubePostDescription, setgeneratedYoutubePostDescription] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setgeneratedYoutubePostDescription([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateYoutubePostDescription?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedYoutubePostDescription(response.data.posts);
        setGeneratedImageUrl(response.data.imageUrl)
      } else {
        toast.error('Error generating post descriptions. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating post descriptions:', error);
      toast.error('Error generating post descriptions. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedYoutubePostDescription.length > 0) {
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedYoutubePostDescription.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "post descriptions.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Youtube post descriptions',
      text: generatedYoutubePostDescription.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing post descriptions:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter Your Youtube Post Description Details</label>
        <textarea
          value={description}
          onChange={(e) => setdescription(e.target.value)}
          placeholder="Describe the content of your YouTube post. 
(e.g.,In this video, we explore the top 5 tips for improving your photography skills.)"
    ...
    <label className="text-lg font-medium text-gray-700">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedYoutubePostDescription.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            generatedYoutubePostDescription.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Youtube Post Description</h1>
              ...
              {generatedYoutubePostDescription.map((post, index) => (
