...
export function GenerateYoutubePostTitle() {
  ...
  const [generatedYotubePostTitle, setgeneratedYotubePostTitle] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateYoutubePostTitle?clerkId=${userId}`, {
        description,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedYotubePostTitle(response.data);
      } else {
        toast.error('Error generating post titles. Please try again later.');
      }
    } catch (error) {
      console.error('Error generating post titles:', error);
      toast.error('Error generating post titles. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && generatedYotubePostTitle.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedYotubePostTitle]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedYotubePostTitle.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "post titles.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'Youtube post titles',
      text: generatedYotubePostTitle.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing post titles:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">What your video is all about ?</label>
        <textarea
          value={description}
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedYotubePostTitle.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      ...
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedYotubePostTitle.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Youtube Post Title</h1>
              ...
              {generatedYotubePostTitle.map((post, index) => (
