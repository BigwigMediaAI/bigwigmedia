...
export function YoutubeScriptOutline() {
  const [isLoading, setIsLoading] = useState(false);
  const [topic, setTopic] = useState('');
  const [tone, setTone] = useState('');
  const [language, setLanguage] = useState('');
  const [outputCount, setOutputCount] = useState(1);
  const [generatedScriptOutlines, setgeneratedScriptOutlines] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateYoutubeScriptOutline?clerkId=${userId}`, {
        topic,
        tone,
        language,
        outputCount,
      ...
      if (response.status === 200) {
        ...
        setgeneratedScriptOutlines(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedScriptOutlines.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedScriptOutlines]);
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedScriptOutlines.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "youtube_script_outline.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedScriptOutlines.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing youtube script outline:', err);
    }
  };
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="Your script topic here"
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedScriptOutlines.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedScriptOutlines.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Youtube Script Outline</h1>
              ...
              {generatedScriptOutlines.map((post, index) => (
