...
export function YoutubeShortsCaption() {
  const [isLoading, setIsLoading] = useState(false);
  const [videoDescription, setvideoDescription] = useState('');
  const [tone, settone] = useState('');
  const [language, setLanguage] = useState('English');
  const [outputCount, setOutputCount] = useState(1);
  const [generatedYoutubeShortsCaption, setgeneratedYoutubeShortsCaption] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    setIsLoading(true);
    setgeneratedYoutubeShortsCaption([]);
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/genrateYoutubeShortsCaption?clerkId=${userId}`, {
        videoDescription,
        tone,
        language,
        outputCount
      ...
      if (response.status === 200) {
        ...
        setgeneratedYoutubeShortsCaption(response.data.GeneratedResponse);
      } else {
        toast.error('Error Generating Youtube Shorts Caption. Please try again later.');
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (!isLoading && generatedYoutubeShortsCaption.length > 0) {
      resultsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  }, [isLoading, generatedYoutubeShortsCaption]);
...
{ value: 'Zulu', label: 'Zulu' }
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedYoutubeShortsCaption.join("\n\n")], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "YtCaption.txt";
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: generatedYoutubeShortsCaption.join("\n\n"),
    };
    try {
      await navigator.share(shareData);
    } catch (err) {
      console.error('Error sharing Youtube-Shorts-Caption:', err);
    }
  };
  ...
  return (
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-md shadow-[var(--teal-color)]">
      ...
      <div className="mb-5">
        <label className="block text-[var(--primary-text-color)]">Enter the Video Description</label>
        <textarea
          value={videoDescription}
          onChange={(e) => setvideoDescription(e.target.value)}
          placeholder="This is an example blog post about AI and its applications in the tech industry. AI is transforming various sectors, including healthcare, finance, and education."
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedYoutubeShortsCaption.length > 0 ? "Regenerate" : 'Generate')}
        </button>
...
          </div>
            ...
            generatedYoutubeShortsCaption.length > 0 && (
            <div ref={resultsRef} className="border border-[var(--primary-text-color)] rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-[var(--primary-text-color)]  ">Generated Youtube Shorts Caption</h1>
              ...
              {generatedYoutubeShortsCaption.map((post, index) => (
