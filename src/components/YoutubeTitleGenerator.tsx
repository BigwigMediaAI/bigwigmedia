...
export function YoutubeTitleGenerator() {
  ...
  const [generatedYtTitles, setGeneratedYtTitles] = useState([]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/response/generateYtTitle?clerkId=${userId}`, {
        topic,
        keywords,
        targetAudience,
        tone,
      ...
      if (response.status === 200) {
        ...
        setGeneratedYtTitles(response.data);
      } else {
  ...
  useEffect(() => {
    if (!isLoading && generatedYtTitles.length > 0) {
  ...
  const handleDownload = () => {
    ...
    const file = new Blob([generatedYtTitles.join("\n\n")], { type: "text/plain" });
    ...
    <div className="m-auto w-full max-w-4xl rounded-lg bg-white p-6 shadow-xl dark:bg-[#262626]">
      ...
      <div className="mb-5">
        ...
        <input
          ...
          placeholder="E.g., YouTube, channel, start, guide"
          className="mt-1 block w-full rounded-md border-gray-300 shadow-sm dark:text-gray-300 p-3 mb-4"
        />
      </div>
      <div className="mb-5">
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Generating...' : (generatedYtTitles.length > 0 ? "Regenerate" : 'Generate')}
        </button>
      </div>
      <div className="mt-5">
        {isLoading ? (
            ...
            generatedYtTitles.length > 0 && (
            <div ref={resultsRef} className="border border-gray-300 rounded-md mt-6 p-5 relative">
                <div className="flex justify-between items-center mb-4">
                <h1 className="text-2xl text-gray-700 dark:text-gray-300 ">Generated Youtube Titles</h1>
              ...
              {generatedYtTitles.map((post, index) => (
